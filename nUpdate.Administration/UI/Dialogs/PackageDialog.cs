...
{
    public partial class PackageDialog : BaseDialog, IAsyncSupportable, IResettable
    {
        ...
        {
            if (categoryTreeView.SelectedNode.Parent == null) // Check whether the selected node is an action or not
        ...
        private async void PackageEditDialog_Load(object sender, EventArgs e)
        {
            ...
            var operations =
                Serializer.Deserialize<IEnumerable<IUpdateAction>>(
                    File.ReadAllText(Path.Combine(_extractedPackagePath, "operations.json"))) ?? Enumerable.Empty<IUpdateAction>();
        ...
        private void AddOperation(IUpdateAction action)
        {
            TreeNode node;
            switch (action.Name)
            {
                case "DeleteFile":
                    var deleteFileAction = action as DeleteFileAction;
                    node = (TreeNode) _deleteNode.Clone();
                    ...
                    if (deleteFileAction != null)
                        deletePage.Controls.Add(new FileDeleteOperationPanel
                        {
                            Path = deleteFileAction.DirectoryPath,
                            ItemList = new JArray(deleteFileAction.FileNames).ToObject<BindingList<string>>()
                        });
                    categoryTabControl.TabPages.Add(deletePage);
                ...
                case "MoveFile":
                    var moveFileAction = action as MoveFileAction;
                    node = (TreeNode) _renameNode.Clone();

                    var renamePage = new TabPage("Move file") {BackColor = SystemColors.Window};
                    if (moveFileAction != null)
                        renamePage.Controls.Add(new FileRenameOperationPanel
                        {
                            SourceFilePath = moveFileAction.SourceFilePath,
                            DestinationFilePath = moveFileAction.DestinationFilePath
                        });
                    categoryTabControl.TabPages.Add(renamePage);
                ...
                case "CreateRegistrySubKey":
                    var createRegistrySubKeyAction = action as CreateRegistrySubKeyAction;
                    node = (TreeNode) _createRegistrySubKeyNode.Clone();
                    ...
                    if (createRegistrySubKeyAction != null)
                        createRegistrySubKeyPage.Controls.Add(new RegistrySubKeyCreateOperationPanel
                        {
                            KeyPath = createRegistrySubKeyAction.RegistryKey,
                            ItemList =
                                new JArray(createRegistrySubKeyAction.SubKeysToCreate).ToObject<BindingList<string>>()
                        });
                    categoryTabControl.TabPages.Add(createRegistrySubKeyPage);
                ...
                case "DeleteRegistrySubKey":
                    var deleteRegistrySubKeyAction = action as DeleteRegistrySubKeyAction;
                    node = (TreeNode) _deleteRegistrySubKeyNode.Clone();
                    ...
                    if (deleteRegistrySubKeyAction != null)
                        deleteRegistrySubKeyPage.Controls.Add(new RegistrySubKeyDeleteOperationPanel
                        {
                            KeyPath = deleteRegistrySubKeyAction.RegistryKey,
                            ItemList =
                                new JArray(deleteRegistrySubKeyAction.SubKeysToDelete).ToObject<BindingList<string>>()
                        });
                    categoryTabControl.TabPages.Add(deleteRegistrySubKeyPage);
                ...
                case "SetRegistryValue":
                    var setRegistryValueAction = action as SetRegistryValueAction;
                    node = (TreeNode) _setRegistryValueNode.Clone();

                    var setRegistryValuePage = new TabPage("Set registry value") {BackColor = SystemColors.Window};
                    if (setRegistryValueAction != null)
                        setRegistryValuePage.Controls.Add(new RegistrySetValueOperationPanel
                        {
                            KeyPath = setRegistryValueAction.RegistryKey,
                            NameValuePairs = new JArray(setRegistryValueAction.ValueTuples)
                                .ToObject<List<Tuple<string, object, RegistryValueKind>>>()
                        });
                    categoryTabControl.TabPages.Add(setRegistryValuePage);
                ...
                case "DeleteRegistryValue":
                    var deleteRegistryValueAction = action as DeleteRegistryValueAction;
                    node = (TreeNode) _deleteRegistryValueNode.Clone();

                    var deleteRegistryValuePage =
                        new TabPage("Delete registry value") {BackColor = SystemColors.Window};
                    if (deleteRegistryValueAction != null)
                        deleteRegistryValuePage.Controls.Add(new RegistryDeleteValueOperationPanel
                        {
                            KeyPath = deleteRegistryValueAction.RegistryKey,
                            ItemList = new JArray(deleteRegistryValueAction.ValueNames).ToObject<BindingList<string>>()
                        });
                    categoryTabControl.TabPages.Add(deleteRegistryValuePage);
                ...
                case "StartProcess":
                    var startProcessAction = action as StartProcessAction;
                    node = (TreeNode) _startProcessNode.Clone();
                    ...
                    if (startProcessAction != null)
                        startProcessPage.Controls.Add(new ProcessStartOperationPanel
                        {
                            Path = startProcessAction.FilePath,
                            Arguments = string.Join(" ", startProcessAction.Arguments)
                        });
                    categoryTabControl.TabPages.Add(startProcessPage);
                    ...
                    break;
                case "StopProcess":
                    var stopProcessAction = action as StopProcessAction;
                    node = (TreeNode) _terminateProcessNode.Clone();
                    ...
                    if (stopProcessAction != null)
                        terminateProcessPage.Controls.Add(new ProcessStopOperationPanel
                        {
                            ProcessName = stopProcessAction.ProcessName
                        });
                    categoryTabControl.TabPages.Add(terminateProcessPage);
                    ...
                    break;
                case "StartService":
                    var startServiceAction = action as StartServiceAction;
                    node = (TreeNode) _startServiceNode.Clone();
                    ...
                    if (startServiceAction != null)
                        startServicePage.Controls.Add(new ServiceStartOperationPanel
                        {
                            ServiceName = startServiceAction.ServiceName,
                            Arguments = new JArray(startServiceAction.Arguments).ToObject<IEnumerable<string>>()
                        });
                    categoryTabControl.TabPages.Add(startServicePage);
                    ...
                    break;
                case "StopService":
                    var stopServiceAction = action as StopServiceAction;
                    node = (TreeNode) _stopServiceNode.Clone();
                    ...
                    if (stopServiceAction != null)
                        stopServicePage.Controls.Add(new ServiceStopOperationPanel
                        {
                            ServiceName = stopServiceAction.ServiceName
                        });
                    categoryTabControl.TabPages.Add(stopServicePage);
                    ...
                    break;
                case "ExecuteScript":
                    var executeScriptAction = action as ExecuteScriptAction;
                    node = (TreeNode) _executeScriptNode.Clone();
        ...
        private void saveButton_Click(object sender, EventArgs e)
        {
            ...
            foreach (
                ...
                select tabPage)
            {
                Popup.ShowPopup(this, SystemIcons.Error, "An added action isn't valid.",
            ...
            var operations = new List<IUpdateAction>();
            foreach (TreeNode node in categoryTreeView.Nodes[4].Nodes)
            {
                ...
                IUpdateAction op = panel.Operation;
